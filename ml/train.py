...
def _load_sequences(word_ids: list) -> tuple[np.ndarray, np.ndarray]:
    ...
    Ordena las muestras de forma determinística por (word_id, sample_id)
    para garantizar resultados reproducibles entre entrenamientos.
    ...
    Returns:
        ...
        y: np.ndarray de labels enteros
    """
    raw = fetch_keypoints_for_words(word_ids)

    # Agrupar por (word_id, sample_id)
    grouped = {}
    ...
    # Orden determinístico: primero por word_id, luego por sample_id
...
def train(epochs: int = 300) -> dict:
    ...
    # Split estratificado 80/20 con seed fijo
